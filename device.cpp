...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_cb(
    ...
    void*                                       pUserData)
{
    string report = to_string( objectType ) + to_string( object ) + ": " + to_string( messageCode ) + ", " + pLayerPrefix + ", " + pMessage;
...
void device::createSurface() { w.createWindowSurface(instance, &surface_); }
...
void device::setup_debug_logger()
{
    if (!enableValidationLayers) return;
    ...
    if (CreateDebugReportCallbackEXT(instance, &debug, nullptr, reinterpret_cast<VkDebugReportCallbackEXT *>(debug_cb)) != VK_SUCCESS) {
        throw runtime_error("failed to setup debug report!");
    }
}
